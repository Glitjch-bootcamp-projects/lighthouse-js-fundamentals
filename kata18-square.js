...
const squareCode = function(message) {
  ...
  const compressedStr = (message.split(" ")).join("");
  const lengthOfStr = compressedStr.length;
  const sqrtStrUp = Math.ceil(Math.sqrt(lengthOfStr));
  ...
  while (i < sqrtStrUp) {
    ...
    let getLetters = "";
    for (let j = 0; j < sqrtStrUp; j++) {
      if (!!compressedStr[j * sqrtStrUp + i]) {
        getLetters += compressedStr[j * sqrtStrUp + i];
      }
    };
