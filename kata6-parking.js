...
const whereCanIPark = function (spots, vehicle) {
  ...
  //Valid spots listed as per vehicle type
  const regularValidSpots = ["R"];
  const smallValidSpots = ["R", "S"];
  const motorcycleValidSpots = ["R", "S", "M"];
  
  //Scan the parking lot using nested loops
  for (let columnX = 0; columnX < spots.length; columnX++) {
    for (let rowY = 0; rowY < spots[columnX].length; rowY++) {
      
      //for regular vehicles, look for only R spots
      if (vehicle === "regular") {
        if (regularValidSpots.includes(spots[columnX][rowY])) {
          availableSpot.push(rowY, columnX);
          return availableSpot;
        } 
      }

      //for small vehicles, look for R, and S spots
      if (vehicle === "small") {
        if (smallValidSpots.includes(spots[columnX][rowY])) {
          availableSpot.push(rowY, columnX);
          return availableSpot;
        } 
      }

      //for motorcycles, look for R, S, and M spots
      if (vehicle === "motorcycle") {
        if (motorcycleValidSpots.includes(spots[columnX][rowY])) {
          availableSpot.push(rowY, columnX);
          return availableSpot;
        } 
      }
  ...
  //If coordinates are still empty after scanning the parking lot, return false.
