...
const checkAir = function (samples, threshold) {
  ...
  let dirtySamples = 0;
  let totalAirUnits = samples.length;

  for (let unit of samples) {
    if (unit === "dirty") {
      dirtySamples += 1;
    }
  }
  
  //If threshold has been exceeded, alarm about the pollution, otherwise declare clean
  if (threshold < (dirtySamples / totalAirUnits)) {
    return "polluted";
  }
  else {
    return "clean";
