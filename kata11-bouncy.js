...
//vvvvvvvEquations startvvvvvvvvvv
const PI = 3.14159 ;
...
//Calculate float by searching for all constituent shapes
const totalVolume = function (solids) {
  let sum = 0;
  
  //Loop through each shape, find its type, and apply the appropriate equation
  for (const solid of solids) {
    if (solid.type === 'sphere') {
      sum += sphereVolume(solid.radius);
    }
    
    if (solid.type === 'cone') {
      sum += coneVolume(solid.radius, solid.height);
    }
    
    if (solid.type === 'prism') {
      sum += prismVolume(solid.height, solid.weight, solid.depth);
    }
  }  
